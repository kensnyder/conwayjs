...
class Cipher {
	...
	public static function usePreset($name) {
		...
		return self::init(self::$presets[$name]);
	}
	...
	/**
	 * Generate ivs. Because iv generation takes so long (e.g. 150ms), ensure we do it once per script
	 * @staticvar array $ivs  generated IVs indexed by iv length
	 * @param int $length  The length to use
	 * @return string
	 */
	protected static function _createIv($length) {
		static $ivs = array();
		if (!isset($ivs[$length])) {
			// use MCRYPT_DEV_URANDOM because it is WAY faster than the default MCRYPT_DEV_RANDOM
			// see http://mbrisby.blogspot.com/2008/06/mcrypt-randomness-in-php.html
			$ivs[$length] = mcrypt_create_iv($length, MCRYPT_DEV_URANDOM);
		}
		return $ivs[$length];
	}
	 ...
	 * 
	 * @return string - String will be 24 characters long
	 */
	public static function slug() {
		$md5 = md5(microtime() . uniqid());
		$shortened = Cipher::baseConvertString($md5, '0123456789abcdef','0123456789bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ');
		if (strlen($shortened) != 24) {
			$md5 = md5(microtime() . uniqid());
			$shortened2 = Cipher::baseConvertString($md5, '0123456789abcdef','0123456789bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ');
			return substr($shortened . $shortened2, 0, 24);
		}
		return $shortened;
	}
	 ...
	 * @return string 
	 */
	public function hash($str, $method = 'sha1') {
		$str = $method($str);
		$str = $this->_baseEncode($str);
		return $str;
	}
